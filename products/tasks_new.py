...
def _import_sample_format(data, supplier):
...
def _import_specification_format(data, supplier):
...
def import_products_from_yaml(file_path, supplier_id):
    """Импорт товаров из YAML файла с поддержкой двух форматов"""
    try:
        ...
        if 'products' in data:
            ...
            imported_count = _import_sample_format(data, supplier)
        elif 'categories' in data and 'goods' in data:
            # Формат спецификации API
            imported_count = _import_specification_format(data, supplier)
        else:
            raise ValueError(
                "Unsupported YAML format. Expected 'products' or "
                "'categories'/'goods' structure."
            )
    ...
    try:
        ...
        for product in products:
            product_data = {
                ...
                'wholesale_price': float(product.price_rrc) if product.price_rrc else float(product.price),
        ...
        file_name = f'products_export_{supplier_id or "all"}.yaml'
        file_path = default_storage.save(f'exports/{file_name}', 
                                       ContentFile(yaml_content.encode('utf-8')))
        
        return f'Экспорт завершен: {file_path}'
